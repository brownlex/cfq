        ...
        <div class="row justify-content-center">
            <div class="col-lg-7 col-md-8">
                <h1 class="text-white text-center " style="color: #fff !important;"><i
                    class="material-icons">looks_4</i>{{ __(' Enter Your Cellphone:') }}</h1>
            </div>

        </div>
        <form method="post" action="{{ route('vistor.create.step.two5') }}" autocomplete="off" class="form-horizontal">
            @csrf
            <div class="container"
                ...
                <div class="row">
                    <label class="col-sm-3 col-form-label text-right"
                        style="color: rgb(0, 0, 0) !important;"><h3>{{ __('Cellphone:') }}</h3></label>
                    <div class="col-sm-3 text-left">
                    <h3><input type="number" class="use-keyboard-input{{ $errors->has('cellphone') ? ' is-invalid' : '' }}"
                        style="width: 255px; border: 1px solid #d2d2d2; border-top: hidden; border-right: hidden; border-left: hidden"
                         ...
                         <span id="cellphone-error" class="error text-danger"
                             for="cellphone">{{ $errors->first('cellphone') }}</span>
                     @endif
                    </div>
