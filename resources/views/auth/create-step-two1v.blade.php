        ...
        <div class="row justify-content-center">
            <div class="col-lg-7 col-md-8">
                <h1 class="text-white text-center " style="color: #fff !important;"><i
                    class="material-icons">looks_3</i>{{ __(' Enter Your Last Name:') }}</h1>
            </div>

        </div>
        <form method="post" action="{{ route('vistor.create.step.two3') }}" autocomplete="off" class="form-horizontal">
            @csrf
            <div class="container"
                ...
                <div class="row">
                    ...
                    <div class="col-sm-3 text-left">
                        <h3><input type="text"
                                class="use-keyboard-input{{ $errors->has('surname') ? ' is-invalid' : '' }}"
                        ...
                        @if ($errors->has('surname'))
                            <span id="surname-error" class="error text-danger"
                                for="surname">{{ $errors->first('surname') }}</span>
                        @endif
                    </div>
